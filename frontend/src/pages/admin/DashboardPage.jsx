...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Fetch district analytics for TN heatmap
  ...
  useEffect(() => {
    if (!selectedDistrict) {
      ...
      setFilteredRecent(
        recent.filter(a =>
          (a.voter?.district || '').toLowerCase() === selectedDistrict.toLowerCase()
        )
      )
    }
  }, [selectedDistrict, recent])
    ...
    <div className="admin-dashboard-view">
      ...
      <div style={{ marginTop: 24, marginBottom: 8 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#02a14d', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          <i className="bi bi-diagram-3-fill" /> Tamil Nadu Local Body & Ward Master Database (DB2) — Live Dynamic Collections
        </div>
      ...
      <div className="admin-card" style={{ marginBottom: 24 }}>
        <div className="admin-card-header">
          <h6 className="admin-card-title">
            <i className="bi bi-geo-alt-fill text-saffron" /> Interactive Tamil Nadu District Map
          </h6>
          <span style={{ fontSize: 12, color: '#8e8b87' }}>Click a district to filter applications</span>
        </div>
        <div style={{ padding: 16 }}>
          <div className="tn-map-section">
            {/* Left: Leaflet Map */}
            <Suspense fallback={
              <div style={{ height: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff8f2', borderRadius: 16, border: '1.5px solid rgba(247,98,1,0.12)' }}>
                <div style={{ textAlign: 'center' }}>
                  <div className="spinner-border text-warning" role="status" style={{ width: '2rem', height: '2rem' }} />
                  <div style={{ marginTop: 8, fontSize: 12, color: '#c0bbb6' }}>Loading Map…</div>
                </div>
              </div>
            }>
              <TamilNaduMap
            ...
            {/* Right: District Ranked List */}
            <div className="tn-district-panel">
              <div className="tn-district-panel-header">
                <i className="bi bi-list-ol text-saffron" style={{ fontSize: 15 }} />
                <span className="tn-district-panel-title">Districts by Applications</span>
                <span className="tn-district-panel-badge">38 Districts</span>
              </div>
              <div className="tn-district-list">
                {(() => {
                  ...
                  if (sorted.length === 0) {
                    return (
                      <div style={{ padding: '32px 16px', textAlign: 'center', color: '#c0bbb6', fontSize: 13 }}>
                        <i className="bi bi-geo-alt" style={{ fontSize: 28, display: 'block', marginBottom: 8, opacity: 0.4 }} />
                  ...
                  return sorted.map(([name, count], idx) => (
                    <div
                      key={name}
                      className={`tn-district-row ${selectedDistrict === name ? 'active' : ''}`}
                      onClick={() => setSelectedDistrict(selectedDistrict === name ? '' : name)}
                    >
                      <span className="tn-district-rank">#{idx + 1}</span>
                      <span className="tn-district-name">{name}</span>
                      <div className="tn-district-bar-wrap">
                        <div className="tn-district-bar" style={{ width: `${Math.round((count / maxCount) * 100)}%` }} />
                      </div>
                      <span className="tn-district-count">{count.toLocaleString()}</span>
                    </div>
                  ))
                })()}
              </div>
      ...
      <div className="admin-card" style={{ marginTop: 24 }}>
        <div className="admin-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <h6 className="admin-card-title">
            <i className="bi bi-clock-history text-saffron" />
            {selectedDistrict ? (
              <>
                Applications —
                <span style={{ color: '#f76201', marginLeft: 6 }}>
                  📌 {selectedDistrict}
                </span>
                <button
                  onClick={() => setSelectedDistrict('')}
                  style={{ marginLeft: 8, background: 'rgba(247,98,1,0.08)', border: '1px solid rgba(247,98,1,0.25)', borderRadius: 10, color: '#f76201', fontSize: 10, fontWeight: 700, padding: '2px 8px', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  ✕ Clear Filter
                </button>
              </>
        ...
        {filteredRecent.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 32, color: 'var(--admin-ink-dim)', fontSize: 13 }}>
            ...
            {selectedDistrict
              ? `No applications found for ${selectedDistrict}.`
